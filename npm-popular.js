...
const CDNJS_PACKAGES_RAW = 'https://raw.githubusercontent.com/cdnjs/packages/master/packages';
  ...
  for (let i = 0; i < cdnjsNames.length; i += PACKAGES_BATCH) {
    ...
    await Promise.all(batch.map(async name => {
      ...
      try {
        ...
        map[name] = (data.autoupdate?.source === 'npm' && data.autoupdate?.target)
...
async function getNpmDownloads(npmNames) {
  ...
  for (const name of scoped) {
    try {
      const { data } = await axios.get(`${NPM_DOWNLOADS_API}/${encodeURIComponent(name)}`);
...
async function getCdnjsFiles(cdnjsName) {
  const { data: lib } = await axios.get(`${CDNJS_API}/${cdnjsName}?fields=version`);
  const version = lib.version;
  const { data: ver } = await axios.get(`${CDNJS_API}/${cdnjsName}/${version}?fields=files`);
...
export async function run() {
  ...
  console.log(`[npm-popular] Resolving npm package names from cdnjs/packages (${cdnjsNames.length} packages)...`);
  ...
  console.log(`[npm-popular] ${renamed.length} name corrections (e.g. ${renamed.slice(0, 3).map(([c, n]) => `${c} → ${n}`).join(', ')})`);
  ...
  const ranked = cdnjsNames
    .map(cdnjsName => ({ cdnjsName, npmName: npmNameMap[cdnjsName], count: downloads[npmNameMap[cdnjsName]] ?? 0 }))
    .filter(p => p.count > 0)
  ...
  ranked.forEach(({ cdnjsName, npmName, count }, i) => {
    const label = cdnjsName !== npmName ? `${cdnjsName} (npm: ${npmName})` : cdnjsName;
    console.log(`  ${String(i + 1).padStart(3)}. ${label}: ${count.toLocaleString()}`);
  });
  ...
  for (const { cdnjsName } of ranked) {
    ...
    if (!files.length) {
      console.log(`[npm-popular] SKIP ${cdnjsName}@${version}: no hashable files`);
      continue;
    }
